...
class User(Base):
    ...
    emp_shift_id = Column(BigInteger, ForeignKey("emp_shift.id"), default=1)
    technology_id = Column(BigInteger, ForeignKey("emp_technology.id"), nullable=True)
...
class EmpSkill(Base):
    ...
    user_id = Column(BigInteger, ForeignKey("users.id"), nullable=False)
    skill_level = Column(Enum(SkillLevel, name="skill_level_enum"))
